...
async function startServer() {
  ...
  app.get("/get", async (req, res) => {
    const index = Math.max(Number(req.query.startIndex), 1);
    const array_query = Array.from({ length: 4 }, (_, i) => i + index);
    const run = (q) => client.execute(q, { prepare: true });
    const result = await Promise.all(
      array_query.map((q) =>
        run(
          `SELECT "Game_ID", "Genre", "Price", "Rate", "Image_path", "Name", "Subtitle" FROM products WHERE "Game_ID" = ${q};`
        )
      )
    );
    res.json({
      result: true,
      data: result.map((r) => r.rows[0]).filter((i) => i !== undefined),
    });
  });
    ...
    function part2(games) {
      if (games === null) {
        ...
        const result = Promise.all(
          games.map((q) =>
            run(
              `SELECT "Game_ID", "Image_path", "Name" FROM products WHERE "Game_ID" = ${q};`
            )
          )
        );
        result
          .then((r) => {
            res.json({
              result: true,
              data: r
                .sort((a, b) => a.rows[0].Game_ID - b.rows[0].Game_ID)
                .map((i) => i.rows[0]),
            });
          })
          .catch((err) => {
  ...
  app.post("/get/payment", async (req, res) => {
    ...
    const result = await Promise.all(
        req.body.data.map((q) =>
            run(
                `SELECT "Game_ID", "Image_path", "Name", "Price" FROM products WHERE "Game_ID" = ${q};`
            )
        )
    );
    res.json({
  ...
  app.post("/order", verifyToken, async (req, res) => {
    ...
    async function updateGame(game_id, user_id) {
      ...
      if (exist === false) {
        ...
        client.execute(query, param, { prepare: true }, (err, result) => {
          if (err) {
            console.error(err);
            res.json({
              result: false,
  ...
  app.get("/rating/avg", async (req, res) => {
    ...
    const query = `SELECT AVG(rating) FROM game_reviews WHERE game_id = ${
      game_id.toString() === "NaN" ? 0 : game_id
    }`;
  ...
  app.post("/searching", async (req, res) => {
    ...
    try {
      ...
      if (genre === undefined) {
        ...
        for (let i = 0; i < array.length; i++) {
          if (array[i].Genre.toLowerCase().includes(genre.toLowerCase())) {
            result2.push(array[i].Game_ID);
          }
        }
      } else {
        for (let i = 0; i < array.length; i++) {
          if (
            ...
            array[i].Genre.toLowerCase().includes(genre.toLowerCase())
          ) {
            result2.push(array[i].Game_ID);
          }
        }
